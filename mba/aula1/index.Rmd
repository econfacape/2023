---
title: "OFERTA, DEMANDA E EQUILÍBRIO DE MERCADO."
...
**Mostrar que os produtores precisam se preocupar com a produção e a oferta e que esta só pode se expandir se houver aumento de demanda.** 


**Caso contrário, o mercado fica em desequilíbrio e se ajusta via preços.**

**Mostrar com dados reais como se pode analisar informações relativas à oferta, demanda e preços de mercado de produtos selecionados**

.pull-left[
```{r echo=FALSE, out.width="75%"}
knitr::include_graphics("https://media.giphy.com/media/Lpdya2DaT7SY2gJsS7/giphy.gif")
```
]

.pull-right[
```{r echo=FALSE, out.width="95%"}
knitr::include_graphics(    "https://media.giphy.com/media/jUK32YfMMHqKc/giphy.gif")
```
]
...
.pull-left[
- A produção agrícola ocorre nas fazendas (que podem ter diferentes tamanhos, pequenas, médias ou grandes);

- A finalidade é usar insumos como terra, trabalho e capital para produzir bens a serem vendidos para outras empresas ou para consumidores.

- O **problema econômico** é: quanto eu vou produzir e quanto vou usar dos vários insumos usados no processo, dada a tecnologia disponível, dados os preços dos insumos e o preço do que estou produzindo? 

- Insumo (fator de produção) é qualquer recurso escasso usado na produção agrícola, podendo ser inteiramente consumido no processo de produção (fertilizante, defensivo, água). 
]

.pull-right[

- Existem também insumos que são recursos que geram fluxos de serviços a serem consumidos (mão de obra, terra, capital).

- Os insumos podem ainda ser fixos ou variáveis e isto vai definir, na Economia, o curto e o longo prazo;

- A combinação dos insumos diversos, considerando um determinado nível tecnológico, vai gerar um certo nível de produção. Esta relação é chamada de **Função de Produção**.

- Produção é algo físico (não monetário) e medida em unidades físicas por período de tempo (na safra 2022 a produção média foi de 24 toneladas por hectare, por exemplo).
...
- Matematicamente a função de produção pode ser definida como $Y=f(x_1, x_2, \dots, x_n)$, em que Y é a quantidade produzida, $x_n$ representa os $n$ insumos usados no processo e $f$ é tecnologia;

- Consequentemente, se tem o produto físico médio (produtividade) dado por $PFM_{x_i}= Y/x_i$

- A Produtividade Marginal, em um determinado ponto, é $PFMg_{x_i}= \partial Y/ \partial x_i$ ;

- E assim pode-se definir a região racional da função de produção como aquela em que um aumento em qualquer insumo não diminui o nível de produção, ou seja, todos os produtos marginais são não negativos; 
]

.pull-right[
- Finalmente, um conceito muito importante é o do retorno a escala. Em linhas gerais, se ocorrer um aumento no uso de insumo e a produção total crescer na mesma proporção, se tem retornos constantes de escala. 

- Se o aumento dos insumos fizer crescer a produção, mas em menor proporção, os retornos de escala são descrescentes.

- Quando os insumos aumentam em determinada proporção e a quantidade produzida cresce mais do que proporcionalmente, os retornos de escala são crescentes. 
]
...
- Exemplo gráfico de uma função de produção tipo Cobb-Douglas 
- Q = quantidades produzidas; L = mão de obra e K = capital.

- Quanto maiores as quantidades dos dois insumos, maior o nível de produção;

- Ao definir um valor para L e outro para K é possível encontrat PFM e PFMg. 

]

.pull-right[
```{r, echo=FALSE, out.width="100%"}
knitr::include_graphics("img_r/fig_oferta1.png")
```
]
...
- Toda a safra vai chegar ao consumidor nas gondolas de supermercados ou feiras livres, etc? 

- Não necessariamente! A oferta pode ser igual ou menor do que a produção!
]
...
- Nesta função, tem-se que $\Delta S_i/\Delta p_i > 0$, ou seja, se o preço do bem aumenta, estimula as empresas a ofertarem mais, *coeteribus paribus*.
]

.pull-right[
- Tem-se que $\Delta S_i/\Delta \pi_m < 0$, ou seja, se o preço do insumo (mão de obra, fertilizantes, terra, etc.) aumenta, leva as empresas a ofertarem menos, *coeteribus paribus*. Ocorre um aumento no custo de produção sem aumento do preço do produto.

- Tem-se que $\Delta S_i/\Delta p_n < 0$, ou seja, se o preço da BRS Melodia aumentar, dado o preço da BRS Vitória, estimula os produtores a produzirem mais BRS Melodia do que BRS Vitória, *coeteribus paribus*. O mesmo aconteceu com a manga Palmer e a Manga Tommy. 

- Tem-se que $\Delta S_i/\Delta O \gtrless 0$, ou seja, se meu objetivo é aumentar *market share* eu posso aumentar minha oferta a preços menores. Se for resultado, eu reduzo minha oferta e aumento preço. 
]
...
.pull-left[
- Qual o formato de uma Curva de Oferta?

- Depende de como a oferta responde ao estímulo dos preços. Se os preços aumentarem 1% e a oferta aumentar 1%, elasticidade preço é *unitária*. Se o aumento da oferta superar o dos preços, é *elástica*. Caso contrário, *inelástica*. Ela pode ser ainda *perfeitamente elástica* (horizontal) ou *perfeitamente inelástica* (vertical). 

- $\epsilon_s =  \dfrac{\partial S_i}{\partial p_i} \dfrac{p_i}{S_i}$

- $\epsilon_s = 1$ elasticidade é unitária; $\epsilon_s > 1$ é elástica; $\epsilon_s < 1$ é inelástica.
]

.pull-right[
```{r, echo=FALSE, out.width="100%"}
knitr::include_graphics("img_r/fig_oferta2.png")
```
]
...
.pull-left[
- Qual o formato de uma Curva de Demanda?

- Depende de como a demanda responde ao estímulo dos preços. Se os preços aumentarem 1% e a demanda reduzir 1%, elasticidade preço é *unitária*. Se o aumento da demanda superar o redução dos preços, é *elástica*. Caso contrário, *inelástica*. Ela pode ser ainda *perfeitamente elástica* (horizontal) ou *perfeitamente inelástica* (vertical). 

- $\epsilon_D =  \dfrac{\partial D_i}{\partial p_i} \dfrac{p_i}{D_i}$

- $\epsilon_D = 1$ elasticidade é unitária; $\epsilon_D > 1$ é elástica; $\epsilon_D < 1$ é inelástica.
]

.pull-right[
```{r, echo=FALSE, out.width="100%"}
knitr::include_graphics("img_r/fig_demanda1.png")
```
]
...
- Nesta função, tem-se que $\Delta D_i/\Delta p_i < 0$, ou seja, se o preço do bem diminui, estimula os consumidores a comprarem mais, *coeteribus paribus*. Chamada **Lei Geral da Demanda**.

- Qual o motivo disto ocorrer? Pelos **efeitos substituição** e **efeito renda**. Quando o preço diminui ele fica mais barato em relação aos concorrentes, cresce a demanda, chamado *efeito substituição*. 

- Quando o preço reduz, o poder aquisitivo dos consumidores aumenta, o consumidor pode comprar mais mercadorias, mesmo que a renda fique constante. É o chamando **efeito renda**.
]

.pull-right[

- Períodos de *inflação* alta, como o ocorrido recentemente, são muito ruins pois reduz o poder aquisitivo dos consumidores, que passam a comprar menos. 

- Produtor as vezes tem dificuldade de entender a relação entre a quantidade demandada e o preço. 

- Em um diálogo, o economista fala: **"Precisa aumentar a demanda, se quiser que os preços aumentem"**. O produtor responde: **"Demanda pela fruta tem, é o atravessador que está pagando pouco para o produtor"**. *Qual a confusão conceitual que está sendo feita?*
]
...
-  Tem-se ainda que $\Delta D_i/\Delta p_s > 0$, ou seja, se o preço de um outro bem (s) aumenta, estimula os consumidores a comprarem mais do bem i, *coeteribus paribus*. Este é o caso dos bens substitutos ou concorrentes. Exemplo: Preço de Manga aumenta, faz crescer a demanda por bananas. 

- Pode ser também que $\Delta D_i/\Delta p_c < 0$, ou seja, se o preço de um outro bem (c) aumenta, desestimula os consumidores a comprarem o bem i, *coeteribus paribus*. Este é o caso dos bens complementares. Exemplo: Morango com creme de leite. Se o preço do creme de leite aumenta, reduz a demanda por morangos. 
]

.pull-right[

- Finalmente, $\Delta D_i/\Delta R$ pode gerar três resultados: 
- $\Delta D_i/\Delta R > 0$ ou seja, **bem normal**, quando a renda aumenta, a demanda aumenta. 

- $\Delta D_i/\Delta R < 0$, chamado de **bem inferior**, quando a demanda aumenta, o consumo cai. 

- $\Delta D_i/\Delta R = 0$, bem de **consumo saciado**, ou seja, mesmo com mais renda, não aumenta a demanda. 
]
...
```{r upe3, out.width  = "100%", fig.align  = "center"}
...
```{r upe4, out.width  = "100%", fig.align  = "center"}
#Direcionado o R para o Diretorio a ser trabalhado
#setwd('c:/Users/Joao Ricardo Lima/Dropbox/tempecon/dados_manga')
setwd('/Users/jricardofl/Dropbox/tempecon/dados_manga')
...
```{r upe5, out.width  = "100%", fig.align  = "center"}
...
```{r upe6, out.width  = "100%", fig.align  = "center"}
#Direcionado o R para o Diretorio a ser trabalhado
...
``` {r upe7, out.width  = "100%", fig.align  = "center"}
#Direcionado o R para o Diretorio a ser trabalhado
setwd('/Users/jricardofl/Dropbox/tempecon/dados_uva')
...
library(lubridate)
anterior <- as.Date("2023-06-01")
atual <-  as.Date("2023-07-01")
...
``` {r upe8, out.width  = "100%", fig.align  = "center"}
...
```{r upe9, out.width  = "100%", fig.align  = "center"}
#Direcionado o R para o Diretorio a ser trabalhado
setwd('/Users/jricardofl/Dropbox/Embrapa/2023/UPE Agro 4.0/2023/aula1/dados')
...
today <- as.Date("2023-08-11") #data para o IGPI - ultima semana do preço semana do preço
mes <- 7
...
``` {r upe10, out.width  = "100%", fig.align  = "center"}
g7 <- ggplot()+
  geom_col(data=tableb2_vol, aes(x=Meses, y=value, fill=variable), lwd=1,
           position = "dodge")+
  scale_fill_manual(values=mycolors)+
...
- Basicamente, preço é a quantia de dinheiro que um comprador dá a um vendedor em troca de um bem ou serviço.

- Em geral, quando os economistas falam em preço, se referem ao **preço de equilíbrio do mercado** – isto é, o preço pelo qual a quantidade de um bem ou serviço ofertado por todos os vendedores em um mercado é igual à quantidade demandada por todos os compradores.

- O ponto em que esses dois preços são iguais é o preço de equilíbrio do mercado. Se um empresário aumentasse os preços para um nível superior ao de equilíbrio do mercado, não venderia todas. Se baixasse os preços teria que recusar clientes, porque ficaria sem conseguir atender todos os pedidos.
...
```{r, echo=FALSE, out.width="95%"}
knitr::include_graphics("img_r/fig3_old.png")
```
...
- Oferta e demanda podem mudar. Por exemplo, se todos os clientes de repente decidissem que gostavam mais de uvas do que antes e estivessem dispostos a pagar um preço mais alto pela mesma quantidade, o preço de equilíbrio do mercado aumentaria. 

- Também poderia aumentar se a oferta de uvas diminuísse – por causa, digamos, de um primeiro semestre muito chuvoso. 

- O preço de equilíbrio de mercado também pode diminuir com mudanças na demanda ou oferta.

- Consideramos um preço único que todos pagam pelo mesmo bem ou serviço. Mas, na realidade, muitos preços diferentes podem existir em um mercado ao mesmo tempo, dependendo das condições em que ocorre uma venda.
]

.pull-right[

```{r echo=FALSE, out.width="95%"}
knitr::include_graphics(    "https://media.giphy.com/media/12P95i4yQfgwJG/giphy.gif")
```
]
...
- Algum estabelecimento pode colocar um produto em promoção, ou seja, preço de venda em menor do que o preço médio.

- Algum estabelecimento pode dar um desconto na segunda unidade da compra do produto ou se for comprado no atacado. 

- Algum estabelecimento pode dar um desconto pela forma de pagamento realizada, se for PIX ou dinheiro tem 5% de desconto na hora do pagamento, por exemplo.

- Quando há muitas empresas de quem comprar e nenhum vendedor individual é capaz de estabelecer o preço, os vendedores são considerados **tomadores de preços**.

]

.pull-right[

- Quando há muitos compradores e muitos vendedores, é mais provável que um único preço de equilíbrio de mercado prevaleça para todos.

```{r echo=FALSE, out.width="75%"}
knitr::include_graphics(    "https://media.giphy.com/media/DED9fIbRcU5yqMf5pT/giphy.gif")
```
]
...
```{r, echo=FALSE, out.width="98%"}
knitr::include_graphics("img_r/fig_mercado1.png")
```
...
``` {r upe11, out.width  = "100%", fig.align  = "center"}
library(imputeTS)
library(tsutils)
data <- as.Date("2023-08-11")
...
``` {r upe12, out.width  = "100%", fig.align  = "center"}
g5 <- ggplot()+
...
``` {r upe13, out.width  = "100%", fig.align  = "center"}
...
``` {r upe14, out.width  = "100%", fig.align  = "center"}
...
``` {r upe15, out.width  = "100%", fig.align  = "center"}
#Direcionado o R para o Diretorio a ser trabalhado
setwd('/Users/jricardofl/Dropbox/tempecon/dados_manga')
...
``` {r upe16, out.width  = "100%", fig.align  = "center"}
#Analise das comparações entre as médias
...
``` {r upe17, out.width  = "100%", fig.align  = "center"}
#Direcionado o R para o Diretorio a ser trabalhado
#setwd('c:/Users/Joao Ricardo Lima/Dropbox/tempecon/dados_manga')
setwd('/Users/jricardofl/Dropbox/tempecon/dados_manga')
...
``` {r upe18, out.width  = "100%", fig.align  = "center"}
#Analise das comparações entre as médias
